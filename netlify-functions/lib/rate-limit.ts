...
//
// Equivalent to the Rust `security` middleware (governor): each IP gets a
// per-minute quota with a burst allowance. Non-exempt paths consume tokens.
// The store is a plain Map with lazy window expiry; it is deliberately not a
// globally shared singleton so tests and multi-instance deployments stay
// isolated.
