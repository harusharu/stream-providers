...
function getDotflixUrl(data: string): string {
  return data.match(/https?:\/\/dotflix\.[^\s"'<>]+\/share\/[a-z\d]+/i)?.[0] || '';
}
...
async function extractDownloadStreams(
  ...
  if (isHubcloudUrl(link)) {
    return hubcloudExtractor(link, signal, providerContext.axios, providerContext.cheerio, {
      ...headers,
    });
  }
  return [];
...
export async function getStream({
  ...
  if (getDotflixSharingCode(page.url) || isHubcloudUrl(page.url)) {
    const streams = await extractDownloadStreams(page.url, signal, headers, providerContext);
  ...
  for (const downloadLink of downloadLinks) {
    const downloadPage = await followDownloadLink(downloadLink.link, signal, headers);
