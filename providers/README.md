...
## Test in the Rust API Gateway

The Rust/Actix gateway (the repo root) replaces the old Node dev server. Provider
development is now done against the Rust API, which spawns Node workers that
execute the compiled bundles.

1. **Build the bundles** (required once, and after every provider change):
   ```sh
   npm run build
   ```

2. **Start the Rust gateway** (see the root `README.md` for full dev setup):
   ```sh
   make dev
   ```

3. **Exercise your provider** through the gateway:
   ```sh
   curl "http://localhost:8787/api/search?provider=<id>&query=test&page=1"
   curl "http://localhost:8787/api/stream?provider=<id>&link=<hub-url>&type=movie"
   ```

4. **Watch provider code rebuild** while developing:
   ```sh
   make watch   # rebuilds bundles on TS change + reloads the API
   ```

The gateway serves every provider at `/api/catalog`, `/api/search`, `/api/meta`,
`/api/episodes`, and `/api/stream`. See the root `README.md` for the full list
of endpoints, configuration, and developer tooling.
