...
// Cinemeta can return garbage names (e.g. a numeric record id) that would
// clobber the correctly-scraped page title. Only trust it when it looks like
// a real title (contains a letter).
function cinemetaTitle(name?: string): string | undefined {
  return name && /[a-zA-Z]/.test(name) ? name : undefined;
}
...
export const getMeta = async ({
  ...
  try {
    ...
    // imdbId
    let imdbId = $('a[href*="imdb.com"]').attr('href')?.match(/tt\d+/)?.[0] || '';
    if (!imdbId) {
      const heading = infoContainer?.find('h3');
      imdbId =
    ...
    if (
      ...
      /\b(?:web\s+series|season\s*\d{1,2}|s\d{1,2}\s*e\d{1,3})\b/i.test(pageText)
    ) {
      type = 'series';
    }
    ...
    list.each((index, element: any) => {
      ...
      // movieLinks
      const movieLinks =
        element?.next().find('.dwd-button').text().toLowerCase().includes('download') ||
      ...
      const episodesLink =
        (vcloudLinks
          ...
          : element?.next().find('.dwd-button').text().toLowerCase().includes('episode')
            ? element?.next().find('.dwd-button')?.parent()?.attr('href')
            : '') || element?.next().find(".btn-outline[style*='#0ebac3']")?.parent()?.attr('href');
    ...
    if (type === 'series' && cinemeta.type === 'series') {
      websiteInfo.linkList = websiteInfo.linkList.map((item) => {
        ...
        return {
          ...item,
          episodesLink: addEpisodeContext(new URL(item.episodesLink, url).href, imdbId, season),
        };
      });
    }
