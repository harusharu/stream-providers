  ...
  try {
    ...
    try {
      ...
      if (error.response?.status === 403) {
        if (openWebView) {
          console.log(`zcloudExtractor: WAF detected (403) for ${link}, using solver...`);
          const cleanHeaders: Record<string, string> = { ...headers, Referer: baseUrl };
          ...
          headers['Cookie'] =
            (headers['Cookie'] ? headers['Cookie'] + '; ' : '') + wafResult.cookies;
          vLinkRes = await axios(`${link}`, { headers, signal });
        } else {
          console.log(
    ...
    try {
      ...
      if (error.response?.status === 403 && openWebView) {
        console.log(`zcloudExtractor: WAF detected (403) for ${vcloudLink}, using solver...`);
        const vcloudBaseUrl = vcloudLink.split('/').slice(0, 3).join('/');
        const cleanHeaders2: Record<string, string> = { ...headers, Referer: vcloudBaseUrl };
        ...
        if (fetchRes.status === 403 && openWebView) {
          console.log(`zcloudExtractor: WAF detected (403) for ${vcloudLink}, using solver...`);
          const vcloudBaseUrl = vcloudLink.split('/').slice(0, 3).join('/');
          const cleanHeaders3: Record<string, string> = { ...headers, Referer: vcloudBaseUrl };
      ...
      switch (true) {
        ...
        default:
          if (link?.includes('.mkv') || link?.includes('?token=')) {
            const serverName =
              link.match(/^(?:https?:\/\/)?(?:www\.)?([^\/]+)/i)?.[1]?.replace(/\./g, ' ') ||
              'Unknown';
