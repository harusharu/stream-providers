  ...
  try {
    ...
    if (targetLink.includes('generate.php')) {
      try {
        ...
        const match = res.data?.match(/window\.location\.href\s*=\s*["'](https?:\/\/[^"']+)["']/i);
        if (match?.[1]) {
          targetLink = match[1];
        }
      } catch (e) {
    ...
    try {
      ...
      if (e.response?.status === 403 && typeof providerContext.openWebView === 'function') {
        const cleanHeaders: Record<string, string> = { ...commonHeaders, Referer: baseUrl };
        ...
        commonHeaders['Cookie'] =
          (commonHeaders['Cookie'] ? commonHeaders['Cookie'] + '; ' : '') + wafResult.cookies;
