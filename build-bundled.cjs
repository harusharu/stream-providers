...
const SKIP_MINIFY = process.env.SKIP_MINIFY === 'true' || process.argv.includes('--dev');
...
async function buildProvider(providerName) {
  ...
  for (const moduleName of modules) {
    ...
    try {
      ...
      // Drop Node builtins that some bundled deps try to require.
...
async function buildAll() {
  ...
  // Mark dist/ as CommonJS so Node doesn't treat the CJS bundles as ESM
